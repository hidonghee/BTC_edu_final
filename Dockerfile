...
RUN mkdir -p /var/log/abclog/gunicorn \
	&& mkdir -p /var/log/abclog/nginx \
	&& apt-get update -y \
	...
	&& sed -i 's/\/var\/log\/nginx\/access.log/\/var\/log\/abclog\/nginx\/access.log/g' /etc/nginx/nginx.conf \
	&& sed -i 's/\/var\/log\/nginx\/error.log/\/var\/log\/abclog\/nginx\/error.log/g' /etc/nginx/nginx.conf
...
CMD ["/bin/bash","-c","venv/bin/gunicorn -b 0:8000 --access-logfile /var/log/abclog/gunicorn/access.log --error-logfile /var/log/abclog/gunicorn/error.log --daemon --reload abcbit.wsgi:application;nginx"]
